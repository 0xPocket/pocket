...
import { PocketFaucet__factory, PocketFaucet, PocketFaucetV2__factory, PocketFaucetV2 } from "../typechain-types";
...
describe('Deploy and tests on proxy functions', function () {
	...
	let PocketFaucetV2_factory: PocketFaucetV2__factory, pocketFaucetV2: PocketFaucetV2;
	...
	it("Should have set good admin for proxy + owner of proxyAdmin", async function () {
		...
		expect(await proxyAdmin.connect(admin).getProxyAdmin(pocketFaucet.address)).to.be.equal(proxyAdmin.address);
	});
	...
	describe('Change owner and proxyAdmin', function () {
		...
		it('Should change proxyAdmin owner', async function () {
			...
			unmute();
			expect(await proxyAdmin.connect(testAccount1).owner()).to.be.equal(testAccount1.address);
	...
	describe('Proxy implementation', function () {
		it('Should return proxy implementation address', async function () {
			const implAddress = await proxyAdmin.connect(admin).getProxyImplementation(pocketFaucet.address);
			const realImplAddr = await upgrades.erc1967.getImplementationAddress(pocketFaucet.address);
	...
	describe('Upgrades', function () {
		it('Should fail to upgrade if badActor', async function () {
			await expect(
				proxyAdmin.connect(badActor).upgrade(pocketFaucet.address, pocketFaucetV2.address))
				.to.be.revertedWith("Ownable: caller is not the owner");
		});

		it('Should upgrade implementation', async function () {
			const pocketV1Addr = await upgrades.erc1967.getImplementationAddress(pocketFaucet.address);
