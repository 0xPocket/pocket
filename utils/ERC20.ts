...
export async function setErc20Balance(
	...
	const tokenContract = new Contract(tokenAddr, ERC20Abi) as IERC20MetadataUpgradeable;
	const decimals = await tokenContract.connect(account).decimals();
	const balance = await tokenContract.connect(account).balanceOf(account.address);
	const newAmount = BigNumber.from(amount).mul(BigNumber.from("10").pow(decimals));

	if (balance.gt(newAmount)) {
		await tokenContract.connect(account).transfer("0x04a8a22e5ef364c5237df13317c4f083f32c2cc4", balance.sub(newAmount));
	}
	else if (balance.lt(newAmount)) {
		const sender = await impersonate(whaleAddr);
		await tokenContract.connect(sender).transfer(account.address, newAmount.sub(balance));
